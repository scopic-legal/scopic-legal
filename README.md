...
# default model is Gemini via Google's OpenAI-compatible endpoint;
...
- **Document Q&A + drafting (DOCX).** Upload a DOCX / PDF / PPTX / XLSX / HTML / EPUB; the agent converts to markdown (DOCX via `mammoth + turndown`, PDF text via `pdfjs-dist`, richer formats via `markitdown-agent`) and answers with section-path citations. For drafting, Counsel proposes a TOC, fills section by section while reading neighbors for coherence, and exports a styled `.docx`. The draft renders live in a read-only artifact panel.
...
- **Settings → model picker** with cloud options (Gemini, Claude, GPT, and optional Qwen via BYOK) plus local models, including Ollama discovery. Per-model routing — picking a Local model swaps the chat call to its own endpoint. Choice persists in localStorage and applies on the next message.
...
| Local models (Qwen / Gemma) | Stand up a vLLM/llama.cpp/unsloth server, set `SCOPIC_LOCAL_QWEN_BASE_URL` and/or `_GEMMA_BASE_URL` | Example ports 8801 + 8802. The model picker routes to these endpoints when a Local row is selected. |
